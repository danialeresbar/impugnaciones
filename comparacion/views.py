...
def JRVViewIncidencia(request, pk):
    ...
    if jrvactual.incidencia_resuelta == True:
        resuelta="si"

    if jrvactual.incidencia_grave == True:
            grave="si"
...
#VALIDACION ASAMBLEA
@login_required(login_url="/login/")
def Validacion_asambleanacional(request, pk_test):
    class valores:
        vcanton = ""
        vprovincia = Provincia.objects.get(codprovincia=pk_test)
        vcount = 0
    ...
    jrv_list = JRV.objects.raw('select * from estructura_jrv where estructura_jrv.no_procede9 <> True and estructura_jrv.para_reclamar9 <> True and estructura_jrv.para_validar9 = True' + donde + ' order by maxdiffnal DESC')
    valores.vcount = len(jrv_list)
    print(jrv_list)
    context = {'valores': valores, 'canton_list': canton_list, "jrv_list": jrv_list}
    return render(request, 'validacion/validacion_nacional.html', context)


#VALIDACION PROVINCIAL
@login_required(login_url="/login/")
def Validacion_asambleaprovincial(request, pk_test):
    class valores:
        vcanton = ""
        vprovincia = Provincia.objects.get(codprovincia=pk_test)
        vcount = 0
    ...
    jrv_list = JRV.objects.raw('select * from estructura_jrv where estructura_jrv.no_procede7 <> True and estructura_jrv.para_reclamar7 <> True and estructura_jrv.para_validar7 = True' + donde + ' order by maxdiffprov DESC')
    valores.vcount = len(jrv_list)
    print(jrv_list)
    context = {'valores': valores, 'canton_list': canton_list, "jrv_list": jrv_list}
    return render(request, 'validacion/validacion_provincial.html', context)


#VALIDACION ANDINO
@login_required(login_url="/login/")
def Validacion_andino(request, pk_test):
    class valores:
        vcanton = ""
        vprovincia = Provincia.objects.get(codprovincia=pk_test)
        vcount = 0
    ...
    jrv_list = JRV.objects.raw('select * from estructura_jrv where estructura_jrv.no_procede8 <> True and estructura_jrv.para_reclamar8 <> True and estructura_jrv.para_validar8 = True' + donde + ' order by maxdiffprov DESC')
    valores.vcount = len(jrv_list)
    print(jrv_list)
    context = {'valores': valores, 'canton_list': canton_list, "jrv_list": jrv_list}
    return render(request, 'validacion/validacion_andino.html', context)
...
#VALIDACION EDIT ASAMBLEA NACIONAL
@login_required(login_url="/login/")
def JRVViewnal(request, pk):
    ...
    if request.method == "POST":
        ...
        q.observaciones=request.POST.get('Observaciones')
        if request.POST.get('reclamar') == 'para_reclamar' :
            q.para_reclamar9 = True
        if request.POST.get('reclamar') == 'No_procede':
            q.no_procede9 = True
        if request.POST.get('quitaron') != '':
            q.quitaron9 = request.POST.get('quitaron')
            q.diffnum9 = True
        if request.POST.get('pusieron') != '':
            q.pusieron9 = request.POST.get('pusieron')
            q.diffnum9 = True
        if request.POST.get('otroreclamo') != '':
            q.otro9 = request.POST.get('otroreclamo')
        q.save()

        response = redirect('/comparacion/validacion_asamblea_nacional/' + str(q.provincia_id))
        return response

    print("version actualizada")
    votacion_list = votacion.objects.raw("select * from comparacion_votacion where dignidad_id=9 and jrv_id='"+pk+"' order by comparacion_votacion.partido_id")
#    votacion_list = votacion.objects.filter(jrv__cod=pk,dignidad_id=1)      #.order_by('partido')

    jrvactual = JRV.objects.get(cod=pk)
    if jrvactual.para_reclamar9 == True:
        print("para reclamar")
        reclamar="si"
    else:
        if jrvactual.no_procede9 == True:
            print("No procede reclamo")
            reclamar="no"
        else:
            reclamar="null"
    ...
    imagencne = requote_uri("https://impugnaciones.andresarauz.ec/images/9-ASAMBLEÍSTAS NACIONALES/" + str(
        jrvactual.provincia_id) + "-" + provstr + "/" + str(jrvactual.canton_id) + "-" + cantonstr + \
                            "/" + str(jrvactual.provincia_id) + "_" + str(
        jrvactual.canton_id) + "_" + circunstr + "_" + str(jrvactual.parroquia_id) + "_" + zonastr + "_" + str(
        jrvactual.numero) + str(jrvactual.genero) + "_" + str(jrvactual.noactapre) + "_I1.pdf")
    print(imagencne)

    context = {"votacion_list": votacion_list, 'jrvactual': jrvactual, 'reclamar': reclamar}
    return render(request, 'validacion/validar_jrv.html', context)

#VALIDACION EDIT ASAMBLEA PROVINCIAL
@login_required(login_url="/login/")
def JRVViewprov(request, pk):
    ...
    if request.method == "POST":
        ...
        q.observaciones=request.POST.get('Observaciones')
        if request.POST.get('reclamar') == 'para_reclamar' :
            q.para_reclamar7 = True
        if request.POST.get('reclamar') == 'No_procede':
            q.no_procede7 = True
        if request.POST.get('quitaron') != '':
            q.quitaron7 = request.POST.get('quitaron')
            q.diffnum7 = True
        if request.POST.get('pusieron')  != '':
            q.pusieron7 = request.POST.get('pusieron')
            q.diffnum7 = True
        if request.POST.get('otroreclamo') != '':
            q.otro7 = request.POST.get('otroreclamo')
        q.save()

        response = redirect('/comparacion/validacion_asamblea_provincial/' + str(q.provincia_id))
        return response

    print("version actualizada")
    votacion_list = votacion.objects.raw("select * from comparacion_votacion where dignidad_id=7 and jrv_id='"+pk+"' order by comparacion_votacion.partido_id")
#    votacion_list = votacion.objects.filter(jrv__cod=pk,dignidad_id=1)      #.order_by('partido')

    jrvactual = JRV.objects.get(cod=pk)
    if jrvactual.para_reclamar7 == True:
        print("para reclamar")
        reclamar="si"
    else:
        if jrvactual.no_procede7 == True:
            print("No procede reclamo")
            reclamar="no"
        else:
            reclamar="null"
    ...
    if jrvactual.zona is None:
        ...
        zonastr = str(jrvactual.zona.codzona)
    imagencne = requote_uri("https://impugnaciones.andresarauz.ec/images/7-ASAMBLEÍSTAS PROVINCIALES/" + str(
        jrvactual.provincia_id) + "-" + provstr + "/" + str(jrvactual.canton_id) + "-" + cantonstr + \
                            "/" + str(jrvactual.provincia_id) + "_" + str(
        jrvactual.canton_id) + "_" + circunstr + "_" + str(jrvactual.parroquia_id) + "_" + zonastr + "_" + str(
        jrvactual.numero) + str(jrvactual.genero) + "_" + str(jrvactual.noactapre) + "_I1.pdf")
...
#VALIDACION EDIT PARLAMENTO ANDINO
@login_required(login_url="/login/")
def JRVViewand(request, pk):
    ...
    if request.method == "POST":
        ...
        q.observaciones=request.POST.get('Observaciones')
        if request.POST.get('reclamar') == 'para_reclamar' :
            q.para_reclamar8 = True
        if request.POST.get('reclamar') == 'No_procede':
            q.no_procede8 = True
            q.diffnum8 = True
        if request.POST.get('quitaron') != '':
            q.quitaron8 = request.POST.get('quitaron')
            q.diffnum8 = True
        if request.POST.get('pusieron') != '':
            q.pusieron8 = request.POST.get('pusieron')
            q.diffnum8 = True
        if request.POST.get('otroreclamo') != '':
            q.otro8 = request.POST.get('otroreclamo')
        q.save()

        response = redirect('/comparacion/validacion_andino/' + str(q.provincia_id))
        return response

    print("version actualizada")
    votacion_list = votacion.objects.raw("select * from comparacion_votacion where dignidad_id=8 and jrv_id='"+pk+"' order by comparacion_votacion.partido_id")
#    votacion_list = votacion.objects.filter(jrv__cod=pk,dignidad_id=1)      #.order_by('partido')

    jrvactual = JRV.objects.get(cod=pk)
    if jrvactual.para_reclamar8 == True:
        print("para reclamar")
        reclamar="si"
    else:
        if jrvactual.no_procede8 == True:
            print("No procede reclamo")
            reclamar="no"
        else:
            reclamar="null"
    ...
    if jrvactual.zona is None:
        ...
        zonastr = str(jrvactual.zona.codzona)
    imagencne = requote_uri("https://impugnaciones.andresarauz.ec/images/8-PARLAMENTARIOS ANDINOS/" + str(
        jrvactual.provincia_id) + "-" + provstr + "/" + str(jrvactual.canton_id) + "-" + cantonstr + \
                            "/" + str(jrvactual.provincia_id) + "_" + str(
        jrvactual.canton_id) + "_" + circunstr + "_" + str(jrvactual.parroquia_id) + "_" + zonastr + "_" + str(
        jrvactual.numero) + str(jrvactual.genero) + "_" + str(jrvactual.noactapre) + "_I1.pdf")
...
@login_required(login_url="/login/")
def Excel_Asamblea_Nacional(request, pk_test):
    response = HttpResponse(content_type='application/ms-excel')
    prov=str(Provincia.objects.get(codprovincia=pk_test)).rstrip("\n")
    response['Content-Disposition'] = 'attachment; filename="RECLAMACION_ASAMBLEA_NACIONAL_'+prov+".xls"
    wb = xlwt.Workbook(encoding='utf-8')
    ws = wb.add_sheet('Nos Quitaron')
    # Sheet header, first row
    row_num = 0
    ...
    # Sheet body, remaining rows
    font_style = xlwt.XFStyle()
    rows = JRV.objects.filter(para_validar=True, quitaron9__isnull= False, provincia_id=pk_test).values_list('canton__nomcanton', 'circunscripcion__codcircunscripcion','parroquia__nomparroquia', 'zona__codzona', 'numero', 'genero', 'quitaron9')
    total=0
    for row in rows:
        row_num += 1
        if row[1] is None: circ="0"
    ...
    # Sheet body, remaining rows
    font_style = xlwt.XFStyle()
    rows = JRV.objects.filter(para_validar=True, pusieron9__isnull=False, provincia_id=pk_test).values_list('canton__nomcanton',
                                                                                     'circunscripcion__codcircunscripcion',
                                                                                     'parroquia__nomparroquia',
                                                                                     'zona__codzona', 'numero',
                                                                                     'genero', 'pusieron9')
    ...
    # Sheet body, remaining rows
    font_style = xlwt.XFStyle()
    rows = JRV.objects.filter(para_validar=True, otro9__isnull=False, provincia_id=pk_test).values_list('canton__nomcanton',
                                                                                     'circunscripcion__codcircunscripcion',
                                                                                     'parroquia__nomparroquia',
                                                                                     'zona__codzona', 'numero',
                                                                                     'genero', 'otro9')
...
@login_required(login_url="/login/")
def Excel_Asamblea_Provincial(request, pk_test):
    response = HttpResponse(content_type='application/ms-excel')
    prov=str(Provincia.objects.get(codprovincia=pk_test)).rstrip("\n")
    response['Content-Disposition'] = 'attachment; filename="RECLAMACION_ASAMBLEA_PROVINCIAL_'+prov+".xls"
    wb = xlwt.Workbook(encoding='utf-8')
    ws = wb.add_sheet('Nos Quitaron')
    # Sheet header, first row
    row_num = 0
    ...
    # Sheet body, remaining rows
    font_style = xlwt.XFStyle()
    rows = JRV.objects.filter(para_validar=True, quitaron7__isnull= False, provincia_id=pk_test).values_list('canton__nomcanton', 'circunscripcion__codcircunscripcion','parroquia__nomparroquia', 'zona__codzona', 'numero', 'genero', 'quitaron7')
    total=0
    for row in rows:
        row_num += 1
        if row[1] is None: circ="0"
    ...
    # Sheet body, remaining rows
    font_style = xlwt.XFStyle()
    rows = JRV.objects.filter(para_validar=True, pusieron7__isnull=False, provincia_id=pk_test).values_list('canton__nomcanton',
                                                                                     'circunscripcion__codcircunscripcion',
                                                                                     'parroquia__nomparroquia',
                                                                                     'zona__codzona', 'numero',
                                                                                     'genero', 'pusieron7')
    ...
    # Sheet body, remaining rows
    font_style = xlwt.XFStyle()
    rows = JRV.objects.filter(para_validar=True, otro7__isnull=False, provincia_id=pk_test).values_list('canton__nomcanton',
                                                                                     'circunscripcion__codcircunscripcion',
                                                                                     'parroquia__nomparroquia',
                                                                                     'zona__codzona', 'numero',
                                                                                     'genero', 'otro7')
...
@login_required(login_url="/login/")
def Excel_Andino(request, pk_test):
    response = HttpResponse(content_type='application/ms-excel')
    prov=str(Provincia.objects.get(codprovincia=pk_test)).rstrip("\n")
    response['Content-Disposition'] = 'attachment; filename="RECLAMACION_PARLAMENTO_ANDINO_'+prov+".xls"
    wb = xlwt.Workbook(encoding='utf-8')
    ws = wb.add_sheet('Nos Quitaron')
    # Sheet header, first row
    row_num = 0
    ...
    # Sheet body, remaining rows
    font_style = xlwt.XFStyle()
    rows = JRV.objects.filter(para_validar=True, quitaron8__isnull= False, provincia_id=pk_test).values_list('canton__nomcanton', 'circunscripcion__codcircunscripcion','parroquia__nomparroquia', 'zona__codzona', 'numero', 'genero', 'quitaron8')
    total=0
    for row in rows:
        row_num += 1
        if row[1] is None: circ="0"
    ...
    # Sheet body, remaining rows
    font_style = xlwt.XFStyle()
    rows = JRV.objects.filter(para_validar=True, pusieron8__isnull=False, provincia_id=pk_test).values_list('canton__nomcanton',
                                                                                     'circunscripcion__codcircunscripcion',
                                                                                     'parroquia__nomparroquia',
                                                                                     'zona__codzona', 'numero',
                                                                                     'genero', 'pusieron8')
    ...
    # Sheet body, remaining rows
    font_style = xlwt.XFStyle()
    rows = JRV.objects.filter(para_validar=True, otro8__isnull=False, provincia_id=pk_test).values_list('canton__nomcanton',
                                                                                     'circunscripcion__codcircunscripcion',
                                                                                     'parroquia__nomparroquia',
                                                                                     'zona__codzona', 'numero',
                                                                                     'genero', 'otro8')
...
##OTRAS CONSULTAS ASAMBLEA NACIONAL

@login_required(login_url="/login/")
def Consulta_Nacional(request, pk_test):
        ...
        mensaje=''
        partidos = partido.objects.filter(dignidad_id=9)
        canton_list = Canton.objects.raw('select * from estructura_canton where estructura_canton.provincia_id='+pk_test)
        ...
        if request.method=="POST":
            ...
            donde='comparacion_votacion.delegados < comparacion_votacion.cne1 and comparacion_votacion.dignidad_id=9 and comparacion_votacion.provincia_id='+pk_test
        ...
        print("antes de enviar siguio")
        context = { 'valores': valores, 'canton_list': canton_list, "votacion_list": votacion_list, 'candidatos': partidos , 'mensaje' : mensaje }
        return render(request, 'consulta/consulta_nacional.html', context)

##OTRAS CONSULTAS PROVINCIAL

@login_required(login_url="/login/")
def Consulta_Provincial(request, pk_test):
        ...
        mensaje=''
        partidos = partido.objects.filter(dignidad_id=7,provincia=pk_test)
        canton_list = Canton.objects.raw('select * from estructura_canton where estructura_canton.provincia_id='+pk_test)
        ...
        if request.method=="POST":
            ...
            donde='comparacion_votacion.delegados < comparacion_votacion.cne1 and comparacion_votacion.dignidad_id=7 and comparacion_votacion.provincia_id='+pk_test
        ...
        print("antes de enviar siguio")
        context = { 'valores': valores, 'canton_list': canton_list, "votacion_list": votacion_list, 'candidatos': partidos , 'mensaje' : mensaje }
        return render(request, 'consulta/consulta_provincial.html', context)

##OTRAS CONSULTAS ANDINO

@login_required(login_url="/login/")
def Consulta_Andino(request, pk_test):
        ...
        mensaje=''
        partidos = partido.objects.filter(dignidad_id=8)
        canton_list = Canton.objects.raw('select * from estructura_canton where estructura_canton.provincia_id='+pk_test)
        ...
        if request.method=="POST":
            ...
            donde='comparacion_votacion.delegados < comparacion_votacion.cne1 and comparacion_votacion.dignidad_id=8 and comparacion_votacion.provincia_id='+pk_test
        ...
        print("antes de enviar siguio")
        context = { 'valores': valores, 'canton_list': canton_list, "votacion_list": votacion_list, 'candidatos': partidos , 'mensaje' : mensaje }
        return render(request, 'consulta/consulta_andino.html', context)
