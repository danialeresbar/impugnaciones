...
from tmp.models import aplicacion
...
class Command(BaseCommand):
    ...
    def create_cne_files(self):
        with open('scripts/basicos/2uploadpresarauz.csv', 'w') as destiny_arauz, open('scripts/basicos/2uploadpreslasso.csv', 'w') as destiny_lasso:
            ...
            with open('scripts/basicos/CNE/resultadosapp.csv', mode='r', encoding='cp1252') as source:
    ...
    def update_jrvs(self):
        ...
        cursor.execute("TRUNCATE tmp_aplicacion")
        insert_count = aplicacion.objects.from_csv('scripts/basicos/2uploadpresarauz.csv', delimiter=",")
        print(f'{insert_count} registros cargados en la BD temporal')
        # Cargar datos de bd temporal a columna correspondiente
        cursor.execute(
            "UPDATE estructura_jrv v SET app_arauz = e.arauz_votes, appsufragantes=e.sufragantes, appnulos=e.nulos, appblancos=e.blancos FROM tmp_aplicacion e WHERE v.cod = e.uid")
        cursor.execute("TRUNCATE tmp_aplicacion")
        insert_count = aplicacion.objects.from_csv('scripts/basicos/2uploadpreslasso.csv', delimiter=",")
        print(f'{insert_count} registros cargados en la BD temporal')
        cursor.execute(
            "UPDATE estructura_jrv v SET app_lasso = e.lasso_votes FROM tmp_aplicacion e WHERE v.cod = e.uid")
